...
class Account_core extends CI_Controller {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($row->banned == 1) {
                    ...
                    $this->session->set_userdata('is_paid', $row->is_paid);
    ...
    public function signup() {
        $data['content'] = load_view('membership_view', '', TRUE);
        $data['alias'] = 'signup';
        load_template($data, $this->active_theme);
    }
    ...
    public function signupform() {
        if ($this->session->userdata('package_id') == '') {
            if (get_settings('classified_settings', 'enable_pricing', 'Yes') == 'Yes')
    ...
    function membership($membership_type = '') {
        $value['membership_type'] = $membership_type;
        $data['content'] = load_view('register_view', $value, TRUE);
        $data['alias'] = 'signup';
        load_template($data, $this->active_theme);
    }
    ...
    function register() {
        ...
        if (empty($_FILES['state_id']['name'])) {
            $this->form_validation->set_rules('state_id', 'State ID', 'required|xss_clean');
        }
        if (empty($_FILES['doc_prec']['name'])) {
            $this->form_validation->set_rules('doc_prec', 'Doctor Prescription', 'required|xss_clean');
        }
        ...
        $this->form_validation->set_rules('membership_type', 'Membership Type', 'required|xss_clean');
        $enable_pricing = get_settings('classified_settings', 'enable_pricing', 'Yes');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->model('admin/package_model');
            $package = $this->package_model->get_package_by_type($membership_type);
            ...
            $config['file_name'] = $this->input->post('username') . "-" . time() . $_FILES['state_id']['name'];
            $this->load->library('upload', $config);

            if (!$this->upload->do_upload('state_id')) {
            ...
            $config1['upload_path'] = './user_uploads/';
            $config1['allowed_types'] = 'gif|jpg|png|pdf';
            $config1['max_size'] = '2048';
            $config1['file_name'] = $this->input->post('username') . "-" . time() . $_FILES['doc_prec']['name'];
            // $this->load->library('upload', $config1);
            $this->upload->initialize($config1);
            if (!$this->upload->do_upload('doc_prec')) {
            ...
            $userdata['state_file'] =        $config['file_name'];
            $userdata['prescription_file'] = $config1['file_name'];
    ...
    function resetpassword($user_name = '', $recovery_key = '') {
        ...
        $query = $this->auth_model->verify_recovery_by_username($user_name, $recovery_key);
    ...
    public function recoverpayment($unique_id) {
        ...
        if ($order->num_rows() > 0) {
            ...
            echo 'payment request id invalid';
            die;
        }
    }
